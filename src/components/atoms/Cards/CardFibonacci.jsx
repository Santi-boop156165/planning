...
const CardFibonacci = ({ number, isSelected, onClick }) => {
  const { isReveal, cards } =  useGameCardStore();
  const countSelections = (number) => {
    const count = cards.filter(
      (card) => card.numberSelected === number
    ).length;
    return count;
  };
  return (
    <>
      <div className="flex flex-col items-center ">
        <div
          ...
          className={`${
            isSelected
              ? "bg-purple-400 -translate-y-3 focus:outline-none"
              : "bg-transparent  hover:animate-flip-x focus:outline-none "
          } cursor-pointer shadow-md  shadow-violet-500/70  h-24 w-14 border border-purple-500 
